...
token = os.getenv('EXTERNAL')  # external tag for service exploring
...
def update_haproxy(ingress_name, ingress_ip, has_issuer):
    repo_path = "/tmp/repo"
    ...
    # Флаг для проверки существующих записей для haproxySubdomain и acmeSubdomain
    found_haproxy = False
    found_acme = False


    # Обновляем haproxySubdomain
    ...
    # Обновляем только если есть cert-manager.io/cluster-issuer аннотация
    if has_issuer:
    ...
    # Если записи не найдены, добавляем новые
    ...
    def scan_existing_ingress():
        ...
        for ingress in ingresses:
            ...
            if annotations.get('kubernetes.io/ingress.class') == 'nginx':
                ...
                print(f"Scan existing - Name: {name}, IP: {ingress_ip}, Has issuer: {has_issuer}")
                update_haproxy(name, ingress_ip, has_issuer)
